	...
	function($scope, REST_SERVICE_TRANSACTIONS, REST_SERVICE_BLOCK)
	{
			...
			REST_SERVICE_TRANSACTIONS.getData($scope.response).then(function(data){
				...
				// updated variables for output
		    		$scope.message = "成功找到交易";
		    		$scope.text1 = "合约ID: " +$scope.info.chaincodeID;
		    		$scope.text2 = "交易ID: " +$scope.info.uuid;
		    		$scope.text3 = "时间戳: " +$scope.info.timestamp.seconds;
		    		$scope.text4 = "纳秒: " +$scope.info.timestamp.nanos;
		    		$scope.text5 = null;
		    		$scope.text6 = null;
		    		$scope.text7 = "时间: " +$scope.info.date ;
			});
			...
			REST_SERVICE_BLOCK.getData($scope.response).then(function(data) {
				if (data) {
					...
					$scope.message = "成功找到区块";
					$scope.text1 = "状态哈希: " + $scope.info.stateHash;
					$scope.text2 = "上一区块的哈希: " + $scope.info.previousBlockHash;
					$scope.text3 = "共识元数据: " + ($scope.info.consensusMetadata || '');
					$scope.text4 = "时间戳: " + $scope.info.nonHashData.localLedgerCommitTimestamp.seconds;
					...
					$scope.text6 = "交易: " + $scope.info.transactions.length;
					$scope.text7 = "时间: " + ($scope.info.date || '');
